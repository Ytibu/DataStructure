...
#include <stdio.h>
#include <stdlib.h>
#include "SeqCQuence.h"

//队列的初始化
void Init(sequence_queue_cqueue *sq) {
    sq->front = sq->rear = 0;
    //front为队首节点在数组中的索引，rear为队尾节点在数组中的索引
}

//判断队列是否为空
int Empty(sequence_queue_cqueue sq) {
    return sq.front == sq.rear?1:0; //队首节点=队尾节点?即判断数组是否为空
}

//输出队列的各个节点值
void Display(sequence_queue_cqueue sq) {
    int i;
...
//取得队列的队首节点值
datatype Get(sequence_queue_cqueue sq) {
    if (Empty(sq)) {
...
//循环队列的插入操作
void Insert_sequence_cqueue(sequence_queue_cqueue *sq, datatype x) {
...
//循环队列的删除操作
void Delete_sequence_cqueue(sequence_queue_cqueue *sq) {
    if (sq->front == sq->rear)
